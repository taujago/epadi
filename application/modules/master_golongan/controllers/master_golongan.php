<?php 
class master_golongan extends op_controller {
	function __construct(){
		parent::__construct();
		$this->load->model("m_master_golongan","dm");
		$this->load->helper("tanggal");
        cek_session_akses('nppt',$this->session->userdata("operator_level"));
	}
	function index(){
		$data['controller'] = get_class($this);
        $data['title'] = "Data Tingkat Perjalanan";
        $data['active'] = "List Data Tingkat Perjalanan";
    ...
    function get_data(){    
        ...
        foreach ($list as $res) {
            $no++;
            $row = array();
            $row["id_golongan"] = $res->id_golongan;
            ...
            $row['cek'] = '<input type="checkbox" class="data-check" value="'.$res->id_golongan.'">';
    ...
    function add(){
        cek_session_akses('master_golongan',$this->session->userdata("operator_level"));
        ...
        if($this->form_validation->run() == TRUE) {
            ...
            $res = $this->db->insert("golongan",$data);
                    if($res) {    
    ...
    function update(){
        ...
        if($this->form_validation->run() == TRUE) {
            ...
            $res  = $this->db->update("golongan",$data);
                    if($res) {    
                    $ret = array("success"=>true,
