    ...
    $.fn.dataTableExt.oApi.fnPagingInfo = function(oSettings)
     ...
     table = $('#datable_1').DataTable({
        ...
        initComplete: function() {
              ...
              processing: true,
              serverSide: true,
              ajax: {"url": "<?php echo site_url("$controller/get_data_anggaran")?>", "type": "POST"},
                  columns: [
                            {
                              "data": "cek",
                               "orderable": false
...
function load_jum_ang(id){
  $.ajax({
    url :'<?php echo site_url("laporan/load_jum_anggaran") ?>/'+id,
    success : function(result) {
      $("#load_jum_ang").html('').append(result);
    }
  });
}
...
function lihat() {
    ...
    save_method = 'update';
    $.ajax({
      'url':'<?php echo site_url() ?>laporan/lihat_mata_anggaran/'+list_id,
      'dataType': 'json',
      'type': 'get',
      'data':{},
      success: function(respose){
         $('#modal_ang').modal('show');
          $("#detail").empty();
          $(".modal-title").empty();
          $.each(respose, function(i, item) {
              $('.modal-title').text("Penggunaan Anggaran "+item.nama_anggaran); 
               trHTML = 
...
function cetak()
{
    ...
    { 
      var l = "L";
      var p = "P";
      window.open('<?php echo site_url("$controller/pdf_mata_anggaran/") ?>/'+list_id+'/'+p);   
...
function semua()
{
      ...
      var l = "L";
      var p = "P";
      window.open('<?php echo site_url("$controller/pdf_semua_mata_anggaran/") ?>/'+p);   
